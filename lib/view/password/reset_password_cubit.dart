...
class ResetPasswordCubit extends Cubit<ResetPasswordState> {
  ...
  void onNewChanged(String value) {
    ...
    final repeatErr = state.repeatPassword.isEmpty
        ? state.repeatError
        : _validateRepeatPassword(state.repeatPassword, value);
    emit(state.copyWith(
        newPassword: value, newError: err, repeatError: repeatErr));
  }

  void onRepeatChanged(String value) {
    final err = _validateRepeat
        ? _validateRepeatPassword(value, state.newPassword)
        : null;
  ...
  void onRepeatUnfocused() {
    final err =
        _validateRepeatPassword(state.repeatPassword, state.newPassword);
  ...
  Future<void> submit() async {
    ...
    try {
      ...
      emit(newState.copyWith(
          status: ResetPasswordStatus.failure, networkFailure: isNetwork));
    }
  }

  String? _validateNewPassword(String v) {
    if (v.isEmpty) return ValidationStrings.fieldEmpty;
