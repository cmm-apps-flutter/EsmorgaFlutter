...
class _EventDetailScreenState extends State<EventDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<EventDetailCubit, EventDetailState>(
      listenWhen: (prev, curr) =>
          prev.effectId != curr.effectId && curr.effectType != null,
      listener: (context, state) {
        switch (state.effectType) {
          ...
          case EventDetailEffectType.openMaps:
            if (state.effectLat != null &&
                state.effectLng != null &&
                state.effectName != null) {
  ...
  Widget _buildBody(
      BuildContext context, EventDetailState state, AppLocalizations l) {
    ...
    final dateStr =
        formatter.format(DateTime.fromMillisecondsSinceEpoch(e.date));
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          EsmorgaText(
              text: e.location.name, style: EsmorgaTextStyle.body1Accent),
          const SizedBox(height: 24),
          EsmorgaText(
              text: l.screenEventDetailsDescription,
              style: EsmorgaTextStyle.heading2),
          const SizedBox(height: 8),
          EsmorgaText(
              text: safeDecode(e.description), style: EsmorgaTextStyle.body1),
          const SizedBox(height: 24),
          EsmorgaText(
              text: l.screenEventDetailsLocation,
              style: EsmorgaTextStyle.heading2),
          const SizedBox(height: 8),
          ...
          EsmorgaButton(
            text: state.isAuthenticated
                ? (e.userJoined ? l.buttonLeaveEvent : l.buttonJoinEvent)
                : l.buttonLoginToJoin,
