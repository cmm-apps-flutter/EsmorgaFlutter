...
-   Responsive Sizing: Do not set fixed pixel dimensions (e.g., `width`, `height`) on content or layout widgets (e.g., images, containers, cards) that are expected to fill or adapt to the available screen space; use proportional layout tools such as `AspectRatio`, `Expanded`, `Flexible`, or `FractionallySizedBox` instead. This does not apply to fixed-spacing widgets such as `SizedBox` used as gaps between elements.
-   Switch Proportionality: Do not use `switch` expressions or statements to branch on an enum (or sealed type) that currently has only a single value; use a plain `if` instead. Reserve `switch` for enums with two or more values where exhaustiveness checking provides real value.
-   Service Lookup Deduplication: Do not call `getIt<T>()` or any service locator lookup repeatedly across multiple methods of the same `State` class; instead, resolve the dependency once in `initState()`, store it in a `late final` field, and reference that field everywhere else.