...
class _FakeFormatter implements EsmorgaDateTimeFormatter {
  @override String formatEventDate(int epochMillis) => 'date';
  @override String formatIsoDateTime(DateTime date, String time) => 'iso';
  @override String formatTimeWithMillisUtcSuffix(int hour, int minute) => 'timeZ';
}

void main() {
  late _MockEventRepository eventRepository; late _MockUserRepository userRepository;
  const testUser = User(name:'John', lastName:'Doe', email:'john@doe.com', role: RoleType.user);
  final joinedEvent = Event(id:'1', name:'Joined', date:0, description:'desc', type:EventType.party, location: const EventLocation(name:'Loc'), userJoined:true, imageUrl:null, tags: const []);

  setUp(() {
    eventRepository = _MockEventRepository(); userRepository = _MockUserRepository();
    if (!getIt.isRegistered<EsmorgaDateTimeFormatter>()) { getIt.registerSingleton<EsmorgaDateTimeFormatter>(_FakeFormatter()); }
  });
